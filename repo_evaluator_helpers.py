...
def is_test_file_path(filename: str, language_config_for_repo: dict) -> bool:
    ...
    if is_test_file_standalone({"file_analysis": language_config_for_repo.get("file_analysis", {})}, filename):
        return True
...
def is_asset_file_path(filename: str, language_config_for_repo: dict) -> bool:
    ...
    return is_asset_file_standalone({"file_analysis": language_config_for_repo.get("file_analysis", {})}, filename)


def get_full_patch_content(repo_full_name: str, base_commit: str, head_commit: str, token: str = None) -> Optional[str]:
    """Fetch full patch content between two commits from GitHub."""
    import requests
    ...
    for line in full_patch.split('\n'):
        if line.startswith('diff --git'):
            ...
            if len(parts) >= 3:
                ...
                if is_source:
                    if not is_test_file_path(filename, language_config) and not is_asset_file_path(filename, language_config):
...
def has_valid_issue_word_count(issue_body_text: str) -> bool:
    """
    Checks if the issue body word count is within the specified range.
    """
