        ...
        for file_path in files:
            ...
            if ext in code_extensions:
                try:
                    ...
                    total_loc += loc
                    rel_path = str(file_path.relative_to(self.repo_path))
                    is_test = any(re.search(pattern, rel_path) for pattern in self.TEST_PATTERNS)
            ...
            try:
                if cov_path.suffix == ".xml":
                    ...
                    if coverage is not None:
                        logger.info(f"Found coverage report: {cov_path} ({coverage:.1f}% coverage)")
                        return coverage
    ...
    def _parse_coverage_json(self, json_path: Path) -> Optional[float]:
        ...
        try:
            ...
            for file_path, file_data in data.items():
                ...
                for _, count in statements.items():
                    total_statements += 1
    ...
    def _calculate_score(
        ...
        if test_coverage is not None:
            ...
            if test_file_ratio >= 0.10:
                ...
                strengths.append(f"Moderate test file ratio ({test_file_ratio*100:.1f}%)")
            else:
            ...
            try:
                ...
                if res.get("errors"):
                    ...
                    if "rate limit" in error_msg.lower() or "403" in error_msg:
                        logger.error(
                            "GitHub API rate limit exceeded. Please provide a GitHub token using --github-token"
                        )
                ...
                for pr_data in pr_nodes:
                    ...
                    if self.start_date and pr_merged_at < self.start_date:
                        ...
                        filter_reason = (
                            f"PR merged {pr_merged_at.date()} before start date "
                    ...
                    if not failed_filter:
                        ...
                        if not (
                            is_english(pr_data.get("title", ""))
                            and is_english(pr_body)
                        ):
                    ...
                    if not failed_filter:
                        ...
                        if not closing_issues:
                            regex_issue_number, _ = extract_issue_number_from_pr_body(
                                pr_body, pr_number
                            )
                            if regex_issue_number:
                                logger.info(
                                    f"PR #{pr_number}: Using regex fallback for issue #{regex_issue_number} via REST API."
                                )
                                try:
                                    issue_data_from_rest = fetch_issue_details_rest(
                                        self.owner,
                                        self.repo_name,
                                        int(regex_issue_number),
                                        self.github_token,
                                    )
                                    ...
                                    logger.warning(
                                        f"Failed to fetch fallback issue #{regex_issue_number} via REST API: {e}"
                                    )
                        ...
                        if closing_issues:
                            ...
                            if not found_valid_issue:
                                ...
                                if issue_typename == "PullRequest":
                                    ...
                                    filter_reason = (
                                        f"Linked issue #{issue_number} is a Pull Request"
                                    )
                                else:
                                    ...
                                    if issue_state != "closed":
                                        ...
                                        filter_reason = (
                                            f"Linked issue #{issue_number} is not closed "
                                            f"(state: {issue_state})"
                                        )
                                    else:
                                        ...
                                        filter_reason = (
                                            f"Issue #{issue_number} word count ({word_count}) "
                                            f"is outside {MIN_ISSUE_WORDS}-{MAX_ISSUE_WORDS} range"
                                        )
                    ...
                    if not failed_filter:
                        ...
                        if len(test_files) < self.min_test_files:
                            ...
                            filter_reason = (
                                f"PR has more than {self.max_non_test_files} non-test files"
                            )
                        ...
                        try:
                            ...
                            if not full_patch:
                                ...
                                if (
                                    ...
                                    filter_reason = (
                                        "Rust files contain embedded tests"
                                    )
                                else:
                                    ...
                                    if not has_sufficient:
                                        ...
                                        filter_reason = (
                                            f"Code changes {source_changes} below "
                                            f"{self.min_code_changes}"
                                        )
                        except Exception as e:
...
class RepoEvaluator:
    ...
    def evaluate(self) -> AnalysisReport:
        ...
        if not language_config:
            ...
            language_config = get_language_config("Unknown")
        ...
        try:
            ...
            if pr_analysis.total_prs == 0:
                ...
                if not self.github_token:
                    logger.warning(
                        "Rate limit exceeded (provide --github-token)"
                    )
...
def print_report(report: AnalysisReport):
    ...
    print(
        "Test frameworks: "
        f"{', '.join(report.repo_metrics.test_frameworks) if report.repo_metrics.test_frameworks else 'None'}"
    )
    ...
    if report.pr_analysis.rejection_breakdown:
        ...
        for filter_name, stats in sorted_rejections:
            print(f" {filter_name}: {stats['count']} ({stats['percentage']}%)")
...
def main():
    ...
    if not repo_path:
        ...
        try:
            repo_path = str(
                clone_repo(f"{owner}/{repo_name}", temp_dir, args.github_token)
            )
        except Exception as e:
    ...
    try:
        ...
        if args.json:
            ...
            if args.output:
                ...
                filepath = output_dir / f"{args.repo.replace('/', '__')}.json"
